...
export type Cart = InferModel<typeof dbCartTable>;
...
async function GET(request: NextRequest) {
  try {
    ...
    const cartData = await db.select().from(dbCartTable);
...
async function POST(request: NextRequest) {
  try {
    ...
    const insertedCartEntry = await db.insert(dbCartTable).values(newCartEntry).returning();
